...
export default function Home() {
  ...
  const [parameters, setParameters] = useState({
    ...
    group: null,
    chart: ["pie", "bar"], // This 'chart' parameter seems to be for the API request, not the chart data itself.
    category: null,
  });
  ...
  return (
    <div className="container mx-auto">
      {/* Logout Button - Assuming it stays in Home.jsx for now or could be moved to a Header component later */}
      <div className="flex">
        <input // Changed from Input component to standard input to avoid prop conflicts if Input is not designed for submit type
      ...
      <div className="mt-5"> {/* Added wrapper for consistent spacing */}
        <h2 className="text-2xl font-semibold text-slate-200 mb-3">Filter Options</h2> {/* Form Section Title */}
        <span className="block text-[16px] font-semibold text-slate-300 mb-3"> {/* Added bottom margin */}
          {query.searched}
        </span>
        <FormComponent
      ...
      {/* Combined "current query" text and status/table/charts section for better flow */}
      <div className="mt-5">
        {(data.length > 0 || status) && ( /* Conditionally render this section only if there's data or a status to show related to current results */
          <>
            <span className="block text-[16px] font-semibold text-slate-300">
              {query.current}
            </span>
            <hr className="my-3 border-slate-600" /> {/* Adjusted margin for HR */}
          </>
        )}

        {status && status !== "Loading" && status !== "Ready" && (
          <p className="w-full my-4 text-center text-[16px] font-semibold text-slate-300"> {/* Changed span to p, adjusted margin */}
            {status}
          </p>
        )}

        {!status && expenses.length > 0 && (
          <div className="overflow-x-auto mt-4"> {/* Added margin to table container */}
            {/* Assuming Table component is already styled well internally or accepts a className for further styling */}
            <Table className="w-full" data={expenses} /> 
          </div>
        )}
        
        {/* ChartsComponent will only render if chart data is available and status is appropriate, as per its internal logic */}
